...
NAME="$1"; PROMPT="$2"; BPM="${3:-125}"; DUR="${4:-90}"; N="${5:-3}"; GRIT="${6:-1.1}"
...
for i in $(seq 1 "$N"); do
  ...
  python -u /mnt/d/flbeat/fl-beat-assimilator/phonkify.py "$G/${NAME}_${i}_raw.wav" "$G/${NAME}_${i}_GRIT.wav" "$GRIT" 2>&1 | grep -E "phonkified"
done
