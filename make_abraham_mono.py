"""MONOPHONIC SF2 (exclusiveClass) = a new note cuts the previous one -> notes connect
seamlessly with NO overlap/overlay. C#6 kept but de-clicked via fade-in. Plus a fully
connected MIDI so notes are as close as possible without ever overlapping."""
...
SLICEDIR = os.path.join(OUT, "slices", "melody_mono")
os.makedirs(SLICEDIR, exist_ok=True)
SR = 44100
SLICE_LEN = 1.6


def build_sf2_mono(slicedir, out_path, name="Abraham Mono"):
    ...
    GENS = 4                                            # keyRange, rootKey, exclusiveClass, sampleID
    ibag = b"".join(struct.pack("<HH", i * GENS, 0) for i in range(N)) + struct.pack("<HH", N * GENS, 0)
    ...
    for i, p in enumerate(pitches):
        ...
        igen += struct.pack("<HH", 57, 1)                                 # exclusiveClass=1 -> MONO
...
SF2 = os.path.join(OUT, "abraham_mono.sf2")
N = build_sf2_mono(SLICEDIR, SF2, "Abraham Mono")
...
def write_mid(name, cap):
    ...
    for i, (s, e, p) in enumerate(evs):
        ...
        end = nxt if cap is None else min(max(e, nxt), s + cap)
        inst.notes.append(pretty_midi.Note(velocity=100, pitch=p, start=s, end=max(end, s + 0.05)))
    pm.instruments.append(inst); pm.write(os.path.join(OUT, name)); print("wrote", name)


write_mid("abraham_mono_legato.mid", None)   # full legato — notes touch (mono cuts overlap)
write_mid("abraham_mono_cap22.mid", 2.2)     # slightly less connected

from sf2utils.sf2parse import Sf2File
v = Sf2File(open(SF2, "rb"))
print("MONO SF2 OK: %d samples, bags=%d  (exclusiveClass=1 -> no overlap)" % (len(v.samples), len(v.instruments[0].bags)))
print("SF2:", SF2)
